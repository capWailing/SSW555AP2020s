...
import unittest
from gedcom_app.control.US1923 import first_cousin_should_not_marry_us19, unique_date_and_birthday_us23
...
class Test(TestCase):
    def test_first_cousin_should_not_marry_us19(self):
        ...
        self.assertEqual([str(error) for error in family_dict['119F5'].error_list],
                         ['Error: FAMILY: US19: 104: 119F5: First cousins marry one another happened in family F5'])

    def test_unique_date_and_birthday_us23(self):
        individual_dict = build_individual_list(path)
        unique_date_and_birthday_us23(individual_dict)
